...
import { AccountSetting, AccountSettings, SettingType } from '../../components/account-settings/configuration/settings';
...
export const SettingInput = (props: Props) => {
  ...
  const RenderSettingString = (values: SettingsValue[], fields: eAccountSettings[] | undefined, disabled: boolean, dataBag: SettingDataBag) => {
    const savingState = savingSetting.get(fields[0]);
    let value = configuration?.get(fields[0]) ?? values[0];

    if (fields[0] == AccountSettings.NoApiName) {
  ...
  const RenderSettingList = (values: SettingsValue[], fields: eAccountSettings[] | undefined, disabled: boolean, dataBag: SettingDataBag) => {
    const savingState = savingSetting.get(fields[0]);
    const value = configuration?.get(fields[0]) ?? values[0];
  ...
  const RenderButton = (values: SettingsValue[] | undefined, disabled: boolean, dataBag: SettingDataBag) => {
    let loading = false;
